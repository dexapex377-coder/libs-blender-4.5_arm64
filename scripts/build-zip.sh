#!/bin/bash
# Build libzip for Android ARM64
set -euo pipefail
NDK_DIR="$1"; OUTPUT_DIR="$2"; BUILD_DIR="$3"; API_LEVEL="${4:-24}"
mkdir -p "$BUILD_DIR" && cd "$BUILD_DIR"
git clone --depth 1 --branch v1.11.1 https://github.com/nih-at/libzip.git src
cd src
cmake -B build \
  ...
  -DBUILD_TOOLS=OFF -DENABLE_GNUTLS=OFF -DENABLE_OPENSSL=OFF
